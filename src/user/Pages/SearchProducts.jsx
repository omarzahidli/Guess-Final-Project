...
import { useFilterProductsQuery } from "../../strore/guessApi"
import ProductCard from "../components/ProductCard"
import { Check, ChevronDown, LayoutGrid, Loader2, Search, SlidersHorizontal, Square, X } from "lucide-react"
...
function List() {
    ...
    useEffect(() => {
        ...
        if (search?.length) {
            sorted = sorted.filter(prod => prod.name.toLowerCase().includes(search.toLowerCase()))
        }
        setSortedData(sorted);
    }, [data, sortProp, search]);
                ...
                {isLoading ? 
                    ...
                    <>
                        <div className="relative px-4 py-3">
                            <Search className="absolute my-4 mx-2 text-gray-400 z-1" />
                            <input onInput={(e) => setSearch(e.target.value)} value={search} type="text" maxLength={50} className="py-4 ps-[39px] pe-16 border rounded w-full" placeholder="Search" />
                            <button onClick={() => setSearch("")} className={`${search?.length ? "inline" : "hidden"} absolute underline text-gray-400 my-4 mx-2 z-1 right-10 top-3`}>Clear</button>
                        </div>
                        <div className="my-4 lg:max-w-[60%] lg:mx-auto">
                            <h2 className="ms-3.5 mb-6 font-bold">Focus On</h2>
                            <div className="flex overflow-auto lg:w-full gap-4">
                                <Link to="/guess/women/clothing?id=16" className="flex flex-col justify-center items-center min-w-[25%] lg:min-w-auto gap-2">
                                    <img className="rounded-full" src="../../01_w.jpg" alt="dresses" />
                                    <p className="uppercase font-bold text-xs">dresses</p>
                                </Link>
                                <Link to="/guess/women/newin?id=14" className="flex flex-col justify-center items-center min-w-[25%] lg:min-w-auto gap-2">
                                    <img className="rounded-full" src="../../02_w.jpg" alt="tops" />
                                    <p className="uppercase font-bold text-xs">tops</p>
                                </Link>
                                <Link to="/guess/women/newin?id=14" className="flex flex-col justify-center items-center min-w-[25%] lg:min-w-auto gap-2">
                                    <img className="rounded-full" src="../../03_w.jpg" alt="beachwear" />
                                    <p className="uppercase font-bold text-xs">beachwear</p>
                                </Link>
                                <Link to="/guess/women/bags?id=18" className="flex flex-col justify-center items-center min-w-[25%] lg:min-w-auto gap-2">
                                    <img className="rounded-full" src="../../04_w.jpg" alt="bags" />
                                    <p className="uppercase font-bold text-xs">bags</p>
                                </Link>
                                <Link to="/guess/women/shoes?id=19" className="flex flex-col justify-center items-center min-w-[25%] lg:min-w-auto gap-2">
                                    <img className="rounded-full" src="../../05_w.jpg" alt="shoes" />
                                    <p className="uppercase font-bold text-xs">shoes</p>
                                </Link>
                                <Link to="/guess/women/occasionwear?id=23" className="flex flex-col justify-center items-center min-w-[25%] lg:min-w-auto gap-2">
                                    <img className="rounded-full" src="../../06_w.jpg" alt="jewellery" />
                                    <p className="uppercase font-bold text-xs">jewellery</p>
                                </Link>
                            </div>
                        </div>
                        <div className="flex justify-between items-center px-6 py-4">
                            <h2 className="text-2xl font-semibold">View All ({isError ? 0 : currentProducts?.length || 0})</h2>
                        ...
                        {isError ? 
                            <div className="py-4 lg:p-6 w-full text-xl">
                            ...
                            <div className={`grid ${grid && "!px-0 !grid-cols-1"} lg:grid-cols-4 md:grid-cols-3 grid-cols-2 gap-1 md:gap-2 py-4 lg:p-6 w-full`}>
                                {currentProducts?.map((prod, idx) => {
                                    return (
                                        <ProductCard key={idx} data={prod} cat={category} parent={prod?.category?.parentId} />
                                    )
                                })}
                            </div>
                        }
