...
import { Loader, X, Rotate3D } from "lucide-react"
import { useEffect, useState } from "react";
import { useEditProductMutation, useGetAllCategoriesQuery, useUploadImagesMutation } from "../../../strore/guessApi";
...
function EditProduct({isProductEditOpen, setIsProductEditOpen, item}) {
            ...
            try {
                ...
                for (const img of previewImages) {
                    if (img.file) {
                        ...
                        previewImages.map(img => uploadedImages.push(img.id))
                    }
                }
                ...
                <div className="space-y-4">
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 max-h-40 overflow-y-auto border p-2 rounded">
                            ...
                            <label key={index} className="flex items-center">
                                <input value={color ?? ""} onChange={handleColorChange} type="checkbox" checked={productColors?.includes(color) ?? ""} className="mr-2" />
                                {color}
                            </label>
                            ))}
                        </div>
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 max-h-40 overflow-y-auto border p-2 rounded">
                            ...
                            <label key={index} className="flex items-center">
                                <input value={size ?? ""} checked={productSizes?.includes(size) ?? ""} onChange={handleSizeChange} type="checkbox" className="mr-2" />
                                {size}
                            </label>
                            ))}
                        </div>
                    ...
                    <div>
                        ...
                        <input value={productSlug ?? ""} onInput={(e) => setProductSlug(e.target.value.toLowerCase().split(' ').join('-'))} type="text" className="w-full p-2 border rounded" placeholder="unique-product-slug" />
                    </div>
                    ...
                    <div className="flex justify-end pt-4">
                        <button onClick={handleEditProduct} type="submit" className="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">
