...
function Nav() {
    const tabs = ["Categories", "Products"]
    const navigate = useNavigate()
    let logo =  <svg version="1.1" xmlns="http://www.w3.org/2000/svg" xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 337.9 69.8" >
                <g>
    ...
    return (
        <header className="sticky z-99 top-0 p-4 dark:bg-gray-100 dark:text-gray-800">
            <div className="container flex justify-between h-16 mx-auto">
                <Link to="/home" title="Back to homepage" className="flex flex-col items-center justify-center cursor-pointer">
                    <div className="w-[100px]">
                        {logo}
                    </div>
                    <h1 className="font-bold text-2xl ">Admin Panel</h1>
                </Link>
                <ul className="items-stretch hidden space-x-10 lg:flex lg:items-center">
                    {tabs.map((tab, idx) => {
                        return(
                ...
                <div className="items-center flex-shrink-0 hidden lg:flex">
                    <button 
                        ...
                        className="self-center px-4 py-2 font-semibold rounded-2xl bg-red-200 transition dura hover:text-white hover:bg-black group">
                        <span className="flex group-hover:animate-bounce">
                            <LogOut /> 
                            Log out
                        </span>
                    </button>
                </div>
                <button className="p-4 lg:hidden cursor-pointer" onClick={() => setModal(true)}>
                    <Menu />
                </button>
            </div>
            <div className={`p-[32px] fixed flex flex-col top-0 left-0 z-10 w-full overflow-hidden h-screen bg-gray-300 lg:hidden ${modal ? 'translate-x-0' : 'translate-x-[100%]'} transition duration-300`}>
                <button className="lg:hidden cursor-pointer self-end" onClick={() => setModal(false)}>
                    <X />
                </button>
